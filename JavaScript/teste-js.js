...
const separarPorIdade = async (todasPessoas)=>{

  function idadeImpar(objetos) {
    if (objetos.idade % 2 == 0) {
      return false
    } else {
      return true
    }
  }
  function idadePar(objetos) {
    if (objetos.idade % 2 === 0) {
      return true
    } else {
      return false
  ...
  let objeto = {nomesIdadePar:todasPessoas.filter(idadePar), nomesIdadeImpar:todasPessoas.filter(idadeImpar)}
  return objeto
}

separarPorIdade(pessoas).then((objeto) => {
  
  // TODO imprimir no console a quantidade de nomes de pessoas com idade par. Saída esperada: 7
  console.log(objeto.nomesIdadePar.length)

  // TODO imprimir no console a quantidade de nomes de pessoas com idade impar. Saída esperada: 7
  console.log(objeto.nomesIdadeImpar.length)
  ...
  console.log(objeto.nomesIdadePar.map((obj)=>{return obj.nome}).sort())
  ...
  console.log(objeto.nomesIdadeImpar.map((obj)=>{return obj.nome}).sort())
